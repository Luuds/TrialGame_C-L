...
public class AsteroidBehavior : MonoBehaviour {
	...
	void RespawnAsteroid(){
		...
		Rigidbody2D clone = (Rigidbody2D) Instantiate(asteroid, viktor, transform.rotation); // Create a new asteroid at a randomly rotated vector.
	...
	void OnParticleCollision (GameObject other){ // This shit happens when it gets hit by filthy plasma particles.
		...
		if (!quantedStateObserved && !isMonster){
			...
			if (body){
				...
				other.GetComponent<Rigidbody2D> ().AddForce (direction * 50f);
			}
		}
